#!/usr/bin/env bash
# ¿Qué? Script de arranque completo del sistema NN Auth (Express Edition).
...
#           Docker. Las variables críticas (DATABASE_URL, MAIL_HOST, etc.) son
...
header "Esperando Backend Express (be)..."
# ¿Qué? Hace polling al endpoint /health del backend hasta recibir HTTP 200.
# ¿Para qué? Confirmar que Express arrancó correctamente y las migraciones se aplicaron.
# ¿Impacto? Si falla, muestra los logs para facilitar el diagnóstico.
BE_URL="http://localhost:3000/health"
attempt=0
